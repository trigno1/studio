    ...
    >
      <div className="relative w-full">
        ...
        <div className="absolute top-1/2 -translate-y-1/2 flex justify-between w-full px-4 pointer-events-none">
          <button
            className="relative z-40 flex h-10 w-10 items-center justify-center rounded-full bg-card/50 backdrop-blur-sm border disabled:opacity-50 pointer-events-auto"
            onClick={scrollLeft}
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <div className="fixed inset-0 z-50 h-screen overflow-auto">
            ...
            >
              <button
                className="sticky top-4 right-0 ml-auto flex h-8 w-8 items-center justify-center rounded-full bg-foreground text-background"
                onClick={handleClose}
              >
      ...
      <motion.div
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-card md:h-[40rem] md:w-96 cursor-pointer"
      >
